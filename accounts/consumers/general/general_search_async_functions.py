...
# models 
from accounts.models import BaseAccount, Principal, Admin, Teacher, Student, Parent
...
def search_group_schedule_schedules(user, role, details):
    try:
        ...
        serialized_schedules = TimetableSerializer(group_schedule.daily_schedules.all(), many=True).data
...
def search_schedule_sessions(details):
    try:
        schedule = Timetable.objects.prefetch_related('sessions').get(schedule_id=details.get('schedule'))
