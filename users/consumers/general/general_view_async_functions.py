...
def view_my_email_address_status_information(user):
    ...
    try:
        ...
        requesting_account = BaseUser.objects.values('email_address', 'email_ban_amount', 'email_banned').get(account_id=user)
        
        # Step 2: Fetch email ban records associated with the user's email, ordered by the most recent ban
        email_bans = EmailBan.objects.filter(email=requesting_account['email_address']).order_by('-banned_at')
