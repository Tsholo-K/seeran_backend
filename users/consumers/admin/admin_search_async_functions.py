...
# serilializers
from users.serializers.general_serializers import BasicAccountDetailsEmailSerializer, SourceAccountSerializer
...
def search_audit_entries(user, role, details):
    try:
        ...
        entries = requesting_account.school.audit_logs.only('actor__name', 'actor__surname', 'outcome', 'object', 'timestamp', 'audit_id').filter(action=details['action'])
    ...
    try:
        ...
        if details['group'] == 'admins':
            group = requesting_account.school.admin_permission_groups.only(
                ...
                models.Prefetch('permissions', queryset=AdminPermission.objects.only('action', 'target_model'))
            ...
            group = requesting_account.school.teacher_permission_groups.only(
                ...
                models.Prefetch('permissions', queryset=TeacherPermission.objects.only('action', 'target_model'))
            ).get(permission_group_id=details['permission_group'])
