...
def update_subject_counts(subject):
    ...
    subject.classroom_count = grade.classrooms.filter(subject=subject).count()
    ...
    subject.save(update_fields=['classroom_count', 'teacher_count', 'student_count'])
...
def update_subject_classrooms_count(subject):
    ...
    subject.classroom_count = grade.classrooms.filter(subject=subject).count()
    subject.save(update_fields=['classroom_count'])
