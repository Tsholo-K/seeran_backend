...
def search_audit_entries(user, role, details):
    try:
        ...
        entries = requesting_account.school.audit_logs.only('actor__name', 'actor__surname', 'outcome', 'target_model', 'timestamp', 'audit_id').filter(action=details['action'])
...
def search_grades(user, role, details):
    try:
        ...
        if details.get('timestamp'):
            ...
            grades = requesting_account.school.grades.filter(models.Q(last_updated__gt=details['timestamp'].isoformat()) | models.Q(timestamp__gt=details['timestamp'].isoformat()))
