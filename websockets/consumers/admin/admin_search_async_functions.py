...
@database_sync_to_async
def search_student_assessment_transcript(user, role, details):
    try:
        # Retrieve the requesting users account and related school in a single query using select_related
        requesting_account = accounts_utilities.get_account_and_linked_school(user, role)
        ...
        if not {'term', 'classroom', 'student', 'assessment'}.issubset(details):
            response = f'could not proccess your request, the provided information is invalid for the action you are trying to perform. please make sure to provide valid student, termt and assessmen IDs and try again'
            audits_utilities.log_audit(actor=requesting_account, action='VIEW', target_model='CLASSROOM', outcome='ERROR', server_response=response, school=requesting_account.school)
            return {'error': response}
...
def search_timetable_sessions(user, role, details):
    try:
        ...
        if role != 'PRINCIPAL' and not permissions_utilities.has_permission(requesting_account, 'VIEW', 'TIMETABLE'):
        ...
        schedule = requesting_account.school.timetables.prefetch_related('sessions').get(timetable_id=details['timetable'])
        serialized_sessions = SessoinsSerializer(schedule.sessions, many=True).data
