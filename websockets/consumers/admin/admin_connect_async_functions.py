...
def account_details(account, role):
    try:
        ...
        # Serialize the user
        serialized_account_details = Serializer(instance=requesting_account).data

        # Return the serialized account details along with unread counts
        return {'websocket_authenticated' : {'account': serialized_account_details, 'messages': unread_messages_count, 'announcements': unread_announcements_count}}
