...
from classrooms.serializers import ClassroomSerializer, ClassroomsSerializer, StudentClassroomSerializer
...
def search_student_assessment_transcript(account, role, details):
    try:
        ...
        # Fetch the specific classroom based on class_id and school
        classroom = student.enrolled_classrooms.get(classroom_id=details['classroom'], register_classroom=False)
        term = classroom.grade.terms.get(term_id=details['term'])
...
def search_student_classroom_card(account, role, details):
    try:
        ...
        serialized_classroom = StudentClassroomSerializer(classroom).data
