...
def form_data_for_updating_assessment(account, role, details):
    try:
        ...
        if details.get('collected'):
            assessment = requesting_account.taught_classrooms.assessments.select_related('moderator').get(assessment_id=details['assessment'])
            serialized_assessment = CollectedAssessmentUpdateFormDataSerializer(assessment).data

            return {"assessment": serialized_assessment}
        
        else:
            assessment = requesting_account.taught_classrooms.assessments.select_related('grade', 'assessor', 'moderator').prefetch_related('grade__terms').get(assessment_id=details['assessment'])
