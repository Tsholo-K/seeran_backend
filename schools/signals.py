...
def update_user_counts_on_delete(sender, instance, **kwargs):
    ...
    if role in ['PRINCIPAL', 'ADMIN', 'TEACHER', 'STUDENT']:
        ...
        if role in ['PRINCIPAL', 'ADMIN']:
            deleted_account.school.admin_count = deleted_account.school.principal.count() + deleted_account.school.admins.count()
        elif role == 'STUDENT':
            deleted_account.school.student_count = deleted_account.school.students.count()
        elif role == 'TEACHER':
            deleted_account.school.teacher_count = deleted_account.school.teachers.count()
